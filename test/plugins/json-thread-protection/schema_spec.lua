...
describe("JSON Thread Protection schema", function()
    it("should work when no configuration has been set", function()
        ...
        assert.truthy(valid)
        assert.falsy(err)
        assert.is_true(config.array_element_count == 20)
        assert.is_true(config.container_depth == 10)
        assert.is_true(config.object_entry_count == 15)
        assert.is_true(config.object_entry_name_length == 50)
        assert.is_true(config.source == "request")
        assert.is_true(config.string_value_length == 500)
    end)
